      ...
      {activeTab !== 'account' && (
        <aside className="w-full md:w-[400px] bg-white border-r border-slate-200 overflow-y-auto max-h-screen flex flex-col shadow-sm">
                ...
                <>
                  <div className="bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100 mb-6">
                    ...
                    <div className="space-y-2">
                      <label className="flex items-center gap-3 p-3 bg-white rounded-xl border border-indigo-100 cursor-pointer hover:border-indigo-400 transition-all">
                        <input type="radio" checked={textGenerationMode === 'fixed'} onChange={() => setTextGenerationMode('fixed')} className="w-4 h-4 accent-indigo-600" />
                        <span className="text-xs font-bold text-slate-800">指定文本内容</span>
                      </label>
                      <label className="flex items-center gap-3 p-3 bg-white rounded-xl border border-indigo-100 cursor-pointer hover:border-indigo-400 transition-all">
                        <input type="radio" checked={textGenerationMode === 'ai'} onChange={() => setTextGenerationMode('ai')} className="w-4 h-4 accent-indigo-600" />
                        <span className="text-xs font-bold text-slate-800">AI 随机生成</span>
                      </label>
                    </div>
                  </div>
                  {textLibraries.map(lib => (
                    <LibrarySection key={lib.id} title={lib.name} isActive={lib.isActive} onToggle={() => toggleLibrary('text', lib.id)} onRemove={() => removeLibrary('text', lib.id)}>
                      <textarea className="w-full h-40 p-3 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none resize-none bg-slate-50" value={lib.content} onChange={(e) => updateTextContent(lib.id, e.target.value)} />
                    </LibrarySection>
                  ))}
                </>
              )}
              ...
              {activeTab === 'beautify' && (
                <div className="space-y-6">
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    ...
                    <input type="text" className="w-full p-3 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none mb-4" value={beautifyUrl} onChange={(e) => setBeautifyUrl(e.target.value)} />
                    <div className="mb-4">
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-2">核心色调</label>
                      <div className="grid grid-cols-2 gap-2">
                        <button onClick={() => setBeautifyConfig(prev => ({ ...prev, primaryColor: 'black' }))} className={`py-2 text-xs font-bold rounded-lg border transition-all ${beautifyConfig.primaryColor === 'black' ? 'bg-slate-800 text-white shadow-md' : 'bg-white text-slate-600 border-slate-200'}`}>沉稳黑色</button>
                        <button onClick={() => setBeautifyConfig(prev => ({ ...prev, primaryColor: 'random' }))} className={`py-2 text-xs font-bold rounded-lg border transition-all ${beautifyConfig.primaryColor === 'random' ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-slate-600 border-slate-200'}`}>绚丽随机</button>
                      </div>
                    </div>
                    <div className="mb-6">
                       <label className="block text-xs font-bold text-slate-500 uppercase mb-3">随机变异程度</label>
                       <div className="flex flex-col gap-2">
                         <div className="flex gap-2">
                           <button onClick={() => setBeautifyConfig(prev => ({ ...prev, randomVariation: 'low' }))} className={`flex-1 py-2.5 text-xs font-bold rounded-xl border transition-all ${beautifyConfig.randomVariation === 'low' ? 'bg-indigo-50 border-indigo-600 text-indigo-700' : 'bg-white text-slate-500'}`}>标准</button>
                           <button onClick={() => setBeautifyConfig(prev => ({ ...prev, randomVariation: 'high' }))} className={`flex-1 py-2.5 text-xs font-bold rounded-xl border transition-all ${beautifyConfig.randomVariation === 'high' ? 'bg-indigo-50 border-indigo-600 text-indigo-700' : 'bg-white text-slate-500'}`}>超高</button>
                         </div>
                         <button onClick={() => setBeautifyConfig(prev => ({ ...prev, randomVariation: 'artistic' }))} className={`w-full py-2.5 text-xs font-bold rounded-xl border transition-all ${beautifyConfig.randomVariation === 'artistic' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white text-slate-500'}`}>艺术（过平台检测）</button>
                       </div>
                    </div>
                    <button onClick={runBeautify} disabled={isBeautifying} className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg transition-all flex items-center justify-center gap-2 transform active:scale-[0.98]">
          ...
          <div className="p-6 border-t border-slate-100 bg-white sticky bottom-0 z-10">
            ...
            <input type="range" min="1" max="100" className="w-full accent-indigo-600 h-1.5 bg-slate-100 rounded-lg appearance-none cursor-pointer mb-6" value={batchCount} onChange={(e) => setBatchCount(parseInt(e.target.value))} />
            <button onClick={startGeneration} disabled={isGenerating} className={`w-full h-14 flex items-center justify-center gap-3 rounded-2xl font-bold text-white transition-all shadow-xl ${isGenerating ? 'bg-slate-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200'}`}>
              {isGenerating ? <LoadingSpinner /> : '立即生成批量图片'}
            </button>
          </div>
        </aside>
      )}
        ...
        {activeTab === 'account' ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
               <div className="md:col-span-1 space-y-6">
                 <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 text-center">
                    ...
                    <span className="px-3 py-1 bg-indigo-600 text-white text-[10px] font-black rounded-full uppercase mt-2 inline-block tracking-widest">{currentUser.role}</span>
                    <hr className="my-8 border-slate-50" />
                    <button onClick={handleLogout} className="w-full py-4 bg-red-50 text-red-600 font-bold rounded-2xl hover:bg-red-100 transition-all">退出系统</button>
                 </div>
               </div>
               <div className="md:col-span-2 space-y-6">
                 <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100">
                    <h3 className="text-lg font-black text-slate-800 mb-8 flex items-center gap-3"><div className="w-1.5 h-6 bg-indigo-600 rounded-full"></div>安全设置</h3>
                    <form onSubmit={handleChangePassword} className="space-y-6">
                      <div className="grid grid-cols-1 gap-6">
                        <div>
                          <label className="block text-xs font-bold text-slate-400 uppercase mb-3 ml-1">当前密码</label>
                          <input type="password" required className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none font-medium" value={passForm.old} onChange={(e) => setPassForm({...passForm, old: e.target.value})} />
                        </div>
                        <div className="grid grid-cols-2 gap-6">
                           <div>
                              <label className="block text-xs font-bold text-slate-400 uppercase mb-3 ml-1">新密码</label>
                              <input type="password" required className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none font-medium" value={passForm.new} onChange={(e) => setPassForm({...passForm, new: e.target.value})} />
                           </div>
                           <div>
                              <label className="block text-xs font-bold text-slate-400 uppercase mb-3 ml-1">确认新密码</label>
                              <input type="password" required className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none font-medium" value={passForm.confirm} onChange={(e) => setPassForm({...passForm, confirm: e.target.value})} />
                           </div>
                        </div>
                      </div>
                      ...
                      <button type="submit" className="px-10 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-bold shadow-xl transition-all transform active:scale-95">更新安全凭据</button>
                    </form>
                 </div>
               </div>
            </div>
          ...
          <>
            <header className="flex items-center justify-between mb-8">
              <div>
                <h1 className="text-3xl font-black text-slate-900 tracking-tight">{activeTab === 'beautify' ? '美化二维码实验室' : '视觉画廊画板'}</h1>
                <p className="text-slate-500 mt-1">{activeTab === 'beautify' ? '提供艺术化处理与码眼样式改造，显著提高素材辨识度' : 'AI智能分词、随机配色与库素材动态排版'}</p>
              </div>
              <div className="flex gap-4">
                {activeTab === 'beautify' && beautifiedQRs.length > 0 && <button onClick={importAllToLibrary} className="px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-bold transition-all shadow-xl shadow-indigo-50 hover:-translate-y-0.5">一键同步到码库</button>}
                {activeTab === 'results' && generatedItems.length > 0 && <button onClick={downloadAll} className="flex items-center gap-2 px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-bold transition-all shadow-xl shadow-emerald-50 hover:-translate-y-0.5"><DownloadIcon size={20} /> 打包下载全部 ({generatedItems.length})</button>}
              </div>
            </header>
            {activeTab === 'beautify' ? (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
                {beautifiedQRs.map((qr, idx) => (
                  <div key={idx} className="group relative bg-white p-3 rounded-[2rem] shadow-sm border border-slate-100 hover:shadow-xl transition-all">
                    ...
                    <div className="absolute inset-0 bg-indigo-900/70 opacity-0 group-hover:opacity-100 transition-opacity rounded-[2rem] flex flex-col items-center justify-center gap-3 p-6 backdrop-blur-[2px]">
                      <button onClick={() => importToLibrary(qr.dataUrl)} className="w-full py-2.5 bg-white text-indigo-900 text-xs font-bold rounded-xl hover:scale-105 transition-transform">导入此枚</button>
                      <a href={qr.dataUrl} download={`qr-art-${idx}.png`} className="w-full py-2.5 bg-indigo-500 text-white text-xs font-bold rounded-xl text-center hover:bg-indigo-400">存为图片</a>
                    </div>
                  </div>
                ))}
                {beautifiedQRs.length === 0 && !isBeautifying && <div className="col-span-full h-[450px] flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-[3rem] bg-white"><MagicIcon size={80} className="text-slate-100 mb-6" /><p className="font-bold text-slate-400 text-lg">点击左侧执行，快速生成艺术化二维码</p></div>}
                {isBeautifying && Array.from({ length: 4 }).map((_, i) => <div key={i} className="aspect-square bg-white animate-pulse rounded-[2rem] p-4 shadow-sm border border-slate-100 flex items-center justify-center"><LoadingSpinner size={40} className="text-indigo-200" /></div>)}
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
                {generatedItems.map((item) => (
                  <div key={item.id} className="group relative bg-white p-4 rounded-[2.5rem] shadow-md border border-slate-100 hover:shadow-2xl transition-all transform hover:-translate-y-2">
                    <img src={item.dataUrl} className="w-full h-auto rounded-[1.8rem]" alt="generated" loading="lazy" />
                    <a href={item.dataUrl} download={`gen-${item.id}.png`} className="absolute top-8 right-8 bg-white/90 backdrop-blur-xl p-4 rounded-2xl shadow-xl opacity-0 group-hover:opacity-100 transition-all hover:bg-indigo-600 hover:text-white"><DownloadIcon size={24} /></a>
                  </div>
                ))}
              </div>
            )}
